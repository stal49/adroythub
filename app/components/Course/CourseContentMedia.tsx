...
const CourseContentMedia = ({
  ...
  const handleReviewReplySubmit = () => {
    ...
    // Helper function to determine if the current video is the last in the last section
    const isLastVideo = () => {
      return activeVideo === data.length - 1;
    };

    const router = useRouter();

    const handlegeneratecertificate = () => {
      router.push(`/coursecertificate/${encodeURIComponent(id)}`)
    }
  
    // Handle the Generate Certificate button click
